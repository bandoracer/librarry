...
high-confidence title, author, and format match is auto-imported into that
...
and stores ranked wanted-item candidates. Review decisions can import, skip, or
reject the pending file. Each review stores explainable evidence in JSON
metadata: source-file facts, filename parsing, local OPF/embedded/audio metadata,
