...
func TestImportCommitsInstalledReleaseAndHistoryAtomically(t *testing.T) {
	...
	if err := db.QueryRow(`select count(*) from notification_deliveries d join notification_events e on e.id=d.event_id where e.event->>'type'='import' and d.target_kind='native'`).Scan(&count); err != nil || count != 1 {
		t.Fatal("recovered import notification lost or duplicated", count, err)
	}
	if err := db.QueryRow(`select count(*) from notification_deliveries where target_kind='compat'`).Scan(&count); err != nil || count != 1 {
		t.Fatal("recovered compatibility notification lost or duplicated", count, err)
	}
