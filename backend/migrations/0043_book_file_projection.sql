-- One snapshot and one definition for per-book evidence and collection queries.
-- NULL selects every recorded book; an empty array selects none. This function
-- describes persisted observations only and never probes the filesystem.
create function librarry_book_file_evidence(wanted_ids uuid[])
