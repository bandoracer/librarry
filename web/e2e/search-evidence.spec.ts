...
test("search shows edition evidence and reviews conflicts without percentage scores", async ({ page }, testInfo) => {
  ...
  await expect(row).toContainText("Edition conflict");
  ...
  await row.click();
  await page.getByText("Book details and sources", { exact: true }).click();
  ...
  await expect(review).not.toContainText("medium confidence");
  expect(adds).toBe(0);
