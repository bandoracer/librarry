...
test("book status explains partial audiobook loss and client outages", async ({ page }, testInfo) => {
 ...
 await expect(page.locator(".library-book-header-badges").getByText("Incomplete", { exact: true })).toBeVisible();
 await expect(page.getByRole("status").filter({ hasText: "Required audiobook media is missing." })).toBeVisible();
 await expect(page.locator(".library-book-header-badges").getByText("Downloaded", { exact: true })).toHaveCount(0);
 state = "unknown";
 await page.reload();
 await expect(page.locator(".library-book-header-badges").getByText("Unknown", { exact: true })).toBeVisible();
 await expect(page.getByRole("status").filter({ hasText: "Download-client evidence is unavailable or incomplete." })).toBeVisible();
 await expect(page.locator(".library-book-header-badges").getByText("Missing", { exact: true })).toHaveCount(0);
