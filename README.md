...
| Calibre integration | Supports Calibre library integration and conversion through Calibre Content Server. | Readarr-style Calibre root-folder settings are accepted, validated, persisted, and returned; actual Calibre Content Server add/convert/delete calls are not implemented yet. | Implement content-server import, conversion, metadata sync, and delete behavior behind the same root-folder contract. |
